...
def test_ward_information_accurate(load_test_data):
    ...
    assert response.context["patient_info"]["total_patients"]() == 5
    assert response.context["patient_info"]["female_patients"]() == 2
    assert response.context["patient_info"]["male_patients"]() == 3
    assert response.context["ward"].available_beds == 22
...
def test_discharge_working(load_test_data):
    ...
    endpoint = reverse(view_name, kwargs={"assignment_id": 193})
    ...
    response = client.get(endpoint, {}, format="html", content_type="text/html")
    assert response.status_code == 200
    assert response.context["patient_info"]["total_patients"]() == 4
    assert response.context["patient_info"]["female_patients"]() == 1
    assert response.context["patient_info"]["male_patients"]() == 3
    assert response.context["ward"].available_beds == 23
    assert response.context["ward"].total_beds == 28
